...
bool Threadpool::createThreadpool(long lMinThreadNum, long lMaxThreadNum)
{
    ...
    for(long i =0; i < lMinThreadNum;i++){
        ...
        if(hThread){
            printf("chreate thread success\n");
            qDebug()<<"chreate thread success\n";
            m_lstThread.push_back(hThread);
        }
        else
        {
            printf("create trhead fail(threadpool)!\n");
            qDebug()<<"create trhead fail(threadpool)!\n";
        }
    }
